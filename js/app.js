...
function generateAIContent(position, requirements, pages) {
    ...
    const techDb = {
        react: {
            ...
            desc: 'Biblioteka JavaScript do budowy interfejsów użytkownika.',
            details: 'Kluczowe koncepty: komponenty (funkcyjne i klasowe), JSX, stan (state), właściwości (props), cykl życia komponentu, hooki (useState, useEffect, useContext, useReducer).',
            project: 'Zbuduj prostą aplikację typu "To-Do List", zarządzając stanem za pomocą hooków. Dodaj możliwość dodawania, usuwania i oznaczania zadań jako ukończone.',
            resources: 'https://react.dev/'
        },
        python: {
            name: 'Python', type: 'language',
            desc: 'Wszechstronny język programowania wysokiego poziomu.',
            details: 'Kluczowe koncepty: typy danych, struktury danych (listy, słowniki, krotki), programowanie obiektowe (klasy, dziedziczenie), generatory, dekoratory, obsługa wyjątków.',
            project: 'Napisz skrypt, który pobiera dane z publicznego API (np. o pogodzie) i zapisuje je w czytelnym formacie do pliku CSV.',
            resources: 'https://www.python.org/'
        },
        sql: {
            name: 'SQL', type: 'database',
            desc: 'Język zapytań do zarządzania relacyjnymi bazami danych.',
            details: 'Kluczowe komendy: SELECT (z WHERE, GROUP BY, ORDER BY), JOIN (INNER, LEFT, RIGHT), INSERT, UPDATE, DELETE, CREATE TABLE. Zrozumienie kluczy głównych i obcych.',
            project: 'Zaprojektuj schemat bazy danych dla prostego bloga (tabele: users, posts, comments) i napisz zapytania, które pobiorą wszystkie posty danego użytkownika wraz z komentarzami.',
            resources: 'https://www.w3schools.com/sql/'
        },
        docker: {
            name: 'Docker', type: 'devops',
            desc: 'Platforma do konteneryzacji aplikacji.',
            details: 'Kluczowe koncepty: obrazy (images), kontenery (containers), Dockerfile, volumes, networking. Różnica między `docker run`, `docker build`, `docker-compose`.',
            project: 'Stwórz Dockerfile dla prostej aplikacji w Pythonie lub Node.js, zbuduj obraz i uruchom go jako kontener.',
            resources: 'https://docs.docker.com/'
        },
        git: {
            name: 'Git', type: 'tools',
            desc: 'Rozproszony system kontroli wersji.',
            details: 'Kluczowe komendy: `commit`, `push`, `pull`, `branch`, `merge`, `rebase`, `log`. Zrozumienie działania gałęzi i rozwiązywania konfliktów.',
            project: 'Załóż nowe repozytorium, stwórz gałąź "feature", wprowadź na niej kilka zmian (commitów), a następnie połącz ją z gałęzią główną (main/master) przez merge.',
    ...
    if (identifiedTech.length > 0) {
        ...
        identifiedTech.forEach(t => {
            html += `<li><strong>${t.name}:</strong> ${pages <= 2 ? t.desc : t.details}</li>`;
        });
        html += `</ul>`;
    } else {
    ...
    // 2. Plan nauki i zadania praktyczne
    if (pages >= 3) {
        html += `<h4 class="mt-4">2. Plan Nauki i Zadania Praktyczne</h4>`;
        html += `<p>Sugerowany plan, aby efektywnie przygotować się do rozmowy:</p><ol>`;
        identifiedTech.forEach(t => {
            html += `<li><strong>${t.name}:</strong> ${t.project}</li>`;
        });
        html += `</ol>`;
    }
    ...
    if (pages >= 5) {
        ...
        if (keywords.includes('react')) html += `<li>Jak działa wirtualny DOM w React i jakie korzyści przynosi?</li><li>Czym są hooki i do czego służy `useEffect`?</li>`;
        if (keywords.includes('python')) html += `<li>Wyjaśnij różnicę między listą a krotką (tuple) w Pythonie.</li><li>Czym są dekoratory i podaj praktyczny przykład ich użycia.</li>`;
        if (keywords.includes('sql')) html += `<li>Opisz różnicę między LEFT JOIN a INNER JOIN.</li>`;
        if (keywords.includes('docker')) html += `<li>Jaka jest różnica między obrazem Docker a kontenerem Docker?</li>`;
        ...
        html += `<li>Jak podchodzisz do nauki nowych technologii i dbasz o aktualność swojej wiedzy?</li>`;
        html += `</ul>`;
    }

    // 4. Dodatkowe Zasoby
    if (pages >= 7) {
        html += `<h4 class="mt-4">4. Dodatkowe Zasoby i Linki</h4>`;
        html += `<ul>`;
        identifiedTech.forEach(t => {
             html += `<li>Dokumentacja <strong>${t.name}</strong>: <a href="${t.resources}" target="_blank">${t.resources}</a></li>`;
        });
        html += `</ul>`;
    }
